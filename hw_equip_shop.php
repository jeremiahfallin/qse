...
//$shield_cost = 250;
$bomb_cost = $cost_bomb;
$sn_cost = (int)($cost_bomb * 100);
$genesis_cost = (int)($cost_bomb * 10);
$terra_c = (int)($cost_bomb * 20);
$terra_t = (int)(ceil(log($terra_c)) * 100);


$rs = "<p><a href=hw_equip_shop.php>Return to $home[equip]</a>";
...
if(isset($buy)) {
	...
	$error_str .= "It is not possible to purchase or use SuperNova Effectors when the Admin Variable for random events is set to less than <b>3</b>. <br />At present it is set to <b class=b1>$random_events</b>";
	...
	} elseif (((!$flag_bomb) || (!$flag_sn_effecter)) && ($user[login_id]!=1)) {
		 $error_str .= "Admin has disabled the purchasing of Bombs. However if you have one you can still use it.<p>";
	} elseif($sure != 'yes' && ($flag_sn_effecter)) {
		...
		take_cash($sn_cost);
		dbn(__FILE__,__LINE__,"update ${db_name}_users set sn_effect = 1 where login_id = '$user[login_id]'");
		}
	...
	$maths=dbr();
		...
		get_var('Load all ships','hw_equip_shop.php',"There are <b>$maths[0]</b> empty fighter bays in <b>$maths[1]</b> ships in this system. <br />Do you want to fill as many as you can afford to fill?",'sure','yes');
		} else {
	...
	} elseif($buy == 12) { // terra imploder
		 if($sure != 'yes') {
			get_var('Buy Terra Imploder',$filename,'Are you sure you want to buy a Terra Imploder?','sure','');
		} else {
			if($user[cash] < $terra_c) {
				$error_str .= "You can not afford a Terra Imploder.<p>";
			} elseif($user[tech] < $terra_t) {
				$error_str .= "You can not afford a Terra Imploder.<p>";
			}else {
				$error_str .= "Genesis device purchased for <b>$terra_c</b> Credits and <b>terra_t</b> Tech<p>";
				take_cash($terra_c);
				take_tech($terra_t);
	...
	$maths2=dbr();
		...
		get_var('Load all ships','hw_equip_shop.php',"There are <b>$maths2[0]</b> empty shield charges in <b>$maths2[1]</b> ships in this system. <br />Do you want to fill as many as you can afford to fill?",'sure','yes');
		} else {
...
$error_str .= Create_PageTitleBlock("$home[equip]");

$error_str .= "<p>$home[equip]:";
...
if($uv_planets <= 0){
	$error_str .= "<br /><a href=$filename?buy=5>Genesis Device</a>: " . number_format($genesis_cost);
}
	$error_str .= "<br /><a href=$filename?buy=12>Terra Imploder</a>: " . number_format($terra_c) . " and $terra_t Tech";
...
if (($flag_bomb) || ($user[login_id] ==1)) {
	...
	if ($random_events == 3 and $flag_sn_effecter) {
		$error_str .= "<br /><a href=$filename?buy=9>SuperNova Effector</a>: " . number_format($sn_cost);
	}
}
