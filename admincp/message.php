...
if($target==-2 && (!isset($clan_id) || $clan_id <= 0)) {
  ...
  print_page("Send Message","You don't have any turns to use.");
	...
	if($target > 0){
		...
		db(__FILE__,__LINE__,"select login_name,login_id,clan_sym from ${db_name}_users where login_id != '$user[login_id]' && (login_id > 5 || login_id = 1) order by login_name asc");
		$dest = dbr();
