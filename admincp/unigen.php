...
if(( !isset($_POST['sure']) ) || ( $_POST['sure'] != 'yes' )){
	...
	if(!isset($preview)) {
		...
		if(!isset($nodirmake)){
			if (!file_exists("$map_path/$db_name")) {
				if (!mkdir("$map_path/$db_name")) {
					print_page('Admin',"<p>Unable to create the $map_path/$db_name directory.<br />"
						. "Make sure $map_path is writable.</p>");
				}
			}
		}
	}
...
//	$UNI['uv_pla_org'] = get_db_var('uv_planet_org');
	...
	$UNI['uv_num_shipyards'] = get_db_var('uv_num_shipyards');
...
//	$bmarks = array();
	...
	if(isset($build_universe) || isset($preview)) {
		...
		if(isset($build_universe)){//generating a new universe
			...
			add_starports_se1($ports);
                        print("Adding Alien ShipYards...<br />");flush();
                        add_shipyards_se1($yards);
			...
			save_universe_se1($systems, $ports, $bmarks, "${db_name}_stars", "${db_name}_ports", $bm_allowed, "${db_name}_shipyards", $yards);
			if($UNI['uv_planets'] >= 1){
