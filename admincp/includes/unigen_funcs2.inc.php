...
function make_systems_1(&$systems) {
	global $UNI,$extinfo,$tables,$systems;
	...
	while(($count = count($systems)) < $UNI['numsystems']) {
		...
		$newsystem['cluster'] = $clus_count;
		$systems[] = $newsystem;
	}
	//exit;
}

function link_systems_2(&$systems) {
        global $UNI,$extinfo;

	$clusters = split_systems($systems);
	print_r($clusters);
        foreach($clusters as $cluster) {
		foreach($cluster as $system){
                set_time_limit(0);
                $numlinks = mt_rand($UNI['minlinks'],$UNI['maxlinks']);
                foreach(get_closest_systems($system,$cluster,$numlinks) as $linksys) {
			print_r($systems[$system['num']]);
			print_r($systems[$linksys['num']]);
			exit;
...
function split_systems($systems)
	{
	foreach($systems as $system)
		{
		$clusters[$system[cluster]] = $system;
		}
	return $clusters;
	}
