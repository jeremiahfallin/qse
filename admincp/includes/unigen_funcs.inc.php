...
//add alien shipyards to the universe.
function add_shipyards_se1(&$yards) {
        global $UNI, $extinfo;
	$max_races = get_db_var('max_races');
        for($i = 0; $i < ($UNI['uv_num_shipyards']); $i++) {
                $new_yard = array('location' => 0);

                $new_yard['location'] = mt_rand(($max_races+1), $UNI['numsystems']);

                //ensure no more than 1 per system. But ONLY if there are enough systems!!!
                if($UNI['uv_num_shipyards'] < $UNI['numsystems']){
                        while(system_has_port($yards, $new_yard)) {
                                $new_yard['location'] = mt_rand(($max_races+1), $UNI['numsystems']);
                        }
                } else {
                        $new_yard['location'] = mt_rand(($max_races+1), $UNI['numsystems']);
                ...
                if($extinfo) {
                        print("<div id='addyards1$new_yard[location]'>-Created SipYard #".($i + 1)." in $new_yard[location]<script>document.all.addyards1$new_yard[location].scrollIntoView();</script></div>");
                        flush();
                }
        }
}
...
//save the universe
function save_universe_se1(&$systems, &$ports, &$bmarks, $table_stars, $table_ports, $table_bms, $table_shipyards, &$yards, $delete=true) {
	global $UNI,$db_name;
	...
	if($delete) {
		...
		if(!empty($table_shipyards)) {
			dbn(__FILE__,__LINE__,"delete from $table_shipyards");
		}
	}
	...
	if(!empty($table_bms)) {
        ...
        if(!empty($table_shipyards)) {
                foreach($yards as $yard) {
                        dbn(__FILE__,__LINE__,"insert into $table_shipyards set location = ".($yard['location'] + 1));
                }
        }
}
