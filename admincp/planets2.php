...
if(isset($sort_planets)){
	...
	$order_by = "order by location asc, fighters desc, planet_name asc";
}
...
db(__FILE__,__LINE__,"select planet_name,location,barren,fighters,colon,cash,tech,metal,fuel,elect,organ,darkmatter "
	. "from ${db_name}_planets where location != 1 && planet_type >= 0 && unigen = 1 $order_by $offset");
$clan_planet = dbr();
if($clan_planet) {
	db2(__FILE__,__LINE__,"select count(*) from ${db_name}_planets where location != 1 && planet_type >= 0 && unigen = 1");
	$total_rows = dbr2();
	...
	$out .= $rs . $page_header . make_table( array(
		...
		"<a href=$PHP_SELF?sort_planets=barren&sorted=$sorted&page=$page>Barren</a>",
	...
	while($clan_planet) {
		...
		$clan_planet['barren'] = $clan_planet['barren'] > 0 ? 'Yes' : 'No';
	...
	$out .= $page > 0 ? "No unigen planets exist on page " . ($page + 1) . '.' : 'No unigen planets exist.';
}
