...
//		$this->auth_code = $GLOBALS['auth_code'];
		$this->url_prefix = $GLOBALS['url_prefix'];
		$this->code_base = $GLOBALS['code_base'];
		$this->gameroot = $GLOBALS['gameroot'];
	...
	{
		db(__FILE__,__LINE__,"select login_id,login_name,last_login,auth,passwd from user_accounts where login_name = '$login_name'");
		$user = dbr();
		return $user;
	}
	...
	function AuthUser($user, $response)
	{
		...
		db(__FILE__,__LINE__,"select challenge from qbase_challenge_record where sessid = '$_sid' order by chal_id desc");
		$chall = dbr();
		$chal = $chall['challenge'];
		$expected_response = md5(strtolower($user['login_name']) . ":" . $user['passwd'] . ":" . $chal);

		// if the response was not not sent - probably means javascript not enabled by browser - in this case
		// fall back to the traditional password comparison. Otherwise continue the more secure challenge/response
		// authentication procedure
		if(empty($_POST['response']))
		{
			echo("<b>Quantum Star SE requires that your browser have javascript enabled. Without javascript the secure challenge/response login process cannot operate, and other in-game functions cannot operate effectively.</b>"); exit();
		}
		elseif($_POST['response'] != $expected_response)
		{
			//insert_history($user['login_id'],"Bad password in login attempt from $REMOTE_ADDR");
			$final_result = FALSE;
		}
		elseif($_POST['response'] == $expected_response)
		{
		...
		if (!isset($user) || !is_array($user) || $final_result === FALSE)
		{
			...
			echo "<br><center><table cellspacing=\"0\" cellpadding=\"0\" width=\"60%\"><tr><td><h2>Login Error!</h2>Either this user does not exist on the server, or else you have entered an incorrect username/password combination.
			<br><br>
			Please note that when logging in you ensure that:<br>
			<ul>
				<li>Your Username uses correct capitalisation, e.g. MyName rather than myname</li>
				<li>Your Password also uses correct capitalisation</li>
				<li>Caps Lock is not enabled on your keyboard</li>
			</ul>
			<br>
			You can now either try again or signup for a free account.<br><br>
			<center><a href=signup_form.php>Sign Up For a FREE Account</a><br><a href=\"".$this->url_prefix."/index.php\">Try to Log In Again</a></center></td></tr></table></center>";
		...
		else //tests are passed, login user and set the cookie
		{
			...
			$expire = time() + 600;
			dbn(__FILE__,__LINE__,"update user_accounts set last_login = ".time().", session_id = '$session', session_exp = '$expire', last_ip = '$last_ip', login_count = login_count + 1 where login_id = '$user[login_id]'");
		}
	...
	function AuthUserGame() {
		...
		if($_SESSION['login_id'] == 1 && !isset($_POST['nd_ad_log'])) //admin login method.
		{
			...
			."<br><center><table cellspacing=\"0\" cellpadding=\"0\" width=\"60%\"><tr><td>"
			...
			."<form method=\"POST\" action=\"$this->url_prefix/login.php\" name=\"admin_login_form\">"
			...
			." - <input type=\"submit\" value=\"Login\" name=\"submit_button\"></form>"
		...
		{ //normal player logging into game
			...
			else
			{
				...
				."<input name=\"ship_name\" value=\"Miner\" size=\"10\">"
				."<br><br>Please enter a nickname for your first Fleet:<br><br><input name=\"fleet_name\" size=\"10\" maxlength=\"10\" minlength=\"3\" value=\"Alpha\">"
...
class Q_AuthCheck extends Q_Authenticate
{
	...
	function Check_Auth($user) {
		if($user['login_id'] == 1 && !preg_match("/game_listing\.php/",$_SERVER['PHP_SELF'])){
			db(__FILE__,__LINE__,"select session_id from se_games where db_name = '$_SESSION[db_name]'");
			$admin_session_id = dbr();
			$user['session_id'] = $admin_session_id['session_id'];
			if(($_SESSION['session_id'] == 0) || ($_SESSION['session_id'] != $user['session_id']) || ($user['session_exp'] < time()))
			{
				...
				echo "<script>self.location='$this->url_prefix/index.php';</script><noscript>You cannot login without JavaScript. Please enable Javascript, or get a browser that supports it.</noscript>";
				exit;
			}
			else
			{
	...
	function Check_Auth_2($user) {
		if($user['login_id'] == 1 && !preg_match("/game_listing\.php/",$_SERVER['PHP_SELF'])){
			if(empty($_SESSION['session_id']) || ($_SESSION['session_id'] != $user['session_id']) || ($user['session_exp'] < time()))
			{
//				echo("1");
				//user session expired;
...
//				echo("2");
		...
		elseif((empty($_SESSION['session_id'])) || ($_SESSION['session_id'] != $user['session_id']) || ($user['session_exp'] < time()))
		{
//			echo("3");
			//user session expired;
...
class Q_ValidateSignup {
	...
	function ValidateSignup($l_name, $first_name, $last_name, $passwd, $passwd_verify, $email_address, $email_address_verify, $disc)
	{
		if(empty($l_name))
		{
			print_header("New Account - $this->game");
			echo "You need to enter a Login Name.";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}

		if(strcmp($l_name,htmlspecialchars($l_name)))
		{
		...
		if(eregi("[^a-z0-9~!@#$%&*_+-=£§¥²³µ¶Þ×€ƒ™ ]",$l_name))
		{
			print_header("New Account - $game");
			echo "eregi";
			echo $l_name;
		...
		{
			print_header("New Account - $this->game");
			echo "You need to enter a last name.";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}
		...
		{
			print_header("New Account - $this->game");
			echo "You must enter a password. Please return to the Sign-Up Form and try again:";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}
		...
		{
			print_header("New Account - $this->game");
			echo "What a good idea. Using your login name as the pass.<br>Use a different password. :)";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}
		...
		{
			print_header("New Account - $this->game");
			echo "Passwords must be at least five characters.";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}
		...
		db(__FILE__,__LINE__,"select login_id from user_accounts where login_name = '$login_name'");
		$user = dbr();
		if($user)
		{
			print_header("New Account - $this->game");
		...
		db(__FILE__,__LINE__,"select login_id from user_accounts where email_address = '$email_address'");
		$user = dbr();
		if($user)
		{
			print_header("New Account - $this->game");
		...
		{
			print_header("New Account - $this->game");
			echo "You must agree to the Server Rules & Disclaimer!";
			echo "<br><br><a href=javascript:history.back()>Back to Sign-up Form</a>";
			print_footer();
			exit();
		}
	...
	{
		$chars='123456789abcdefghijklmnpqrstuvwxyzABCDEFGHUJKLMNPQRSTUVWXYZ';
		$retval='';
		for ($i=0 ; $i<12 ; $i++)
		{
