...
switch($install_step)
{
	...
	case "2":
		...
		if(!empty($_POST['keep_users']) && $_POST['keep_users'] == 1)
		{
			...
			flush();
			ExecSQL ("new_server.sql", "", "");
			ExecSQL ("se_svr_star_names.sql", "", "");
//			if(empty($_POST['keep_tracker']))
//			{
//				ExecSQL ("server_issuetracking.sql", "", "");
//			}
			$p_md5admpass = md5($p_admpass);
			dbn(__FILE__,__LINE__,"update user_accounts set passwd = '$p_md5admpass' where login_id = 1");
			dbn(__FILE__,__LINE__,"update user_accounts set passwd = '$p_md5admpass' where login_id = 5");
