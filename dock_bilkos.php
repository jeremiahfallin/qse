...
$text .= "<br /><a href=dock_bilkos.php?view=2>Equipment</a> - (<b>$out[2]</b>)";
$text .= "<br /><a href=dock_bilkos.php?view=3>Upgrades</a> - (<b>$out[3]</b>)";
$text .= "<br /><a href=dock_bilkos.php?view=4>Misc</a> - (<b>$out[4]</b>)";
$text .= "<br /><a href=dock_bilkos.php?view=5>Planetary</a> - (<b>$out[5]</b>)";


db(__FILE__,__LINE__,"select count(item_id) from ${db_name}_bilkos where active=0 && bidder_id='$user[login_id]'");
$do_now=dbr();
if($do_now[0]){
	$text .= "<p><a href=dock_bilkos.php?show_won=1>Collect Item(s)</a> - (<b>$do_now[0]</b>)";
}
...
if($collect){
		...
		if($item['item_type'] == 3){ // upgrades
			...
			if($destination){
				...
				if(!$planets){
					...
					$text .= "<form method=post action=dock_bilkos.php name=despatch_form>";
	...
	if($item[active] = 0){
		...
		$text .= "<form method=post action=dock_bilkos.php name=bid_form>";
		...
		$rs = "<a href=dock_bilkos.php>Back to Bilkos</a>";
	...
	if(!$items){
		...
		while($items) {
			...
			$items[item_id] = " - <a href=dock_bilkos.php?bid=$items[item_id]>Bid</a>";
	...
	if($collect){
		...
		while($collect) {
			...
			$text .= make_hash_row(array($collect[item_name],$collect[item_type],number_format($collect[going_price]),
				" - <a href=dock_bilkos.php?collect=$collect[item_id]>Collect</a>"));
			$collect = dbr2(1);
		}
		$text .= "</table>";
	} else {
